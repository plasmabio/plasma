...
By default the Ansible playbooks use a pinned version of the `jupyterhub/repo2docker`, which is
defined in the `ansible/vars/default.yml` file.

Like other Ansible variables, it is possible to use a different version of `repo2docker` by overriding
the variable via the command line:
